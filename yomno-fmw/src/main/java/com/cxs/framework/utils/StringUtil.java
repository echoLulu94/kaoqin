...
public class StringUtil {
	 ...
	 * @param init 补位数据
	 * 			例如:
	 * 				1.总长度8位，左补零初始值则为8个0："00000000"
	 * 				2.总长度6位，左补A初始值则为6个A："AAAAAA"
	 * @return
	 */
	public static String leftFillUp(String youNumber,String init){
		if(null!=youNumber&&!"".equals(youNumber)&&null!=init&&!"".equals(init)){
			if(init.length()>youNumber.length()){
				youNumber=init.substring(0, init.length()-youNumber.length())+youNumber;
			}
		}else{
			youNumber=init;
		}
    ...
    public static boolean isEmpty(String str) {
        return null == str || "".equals(str.trim()) || "\t".equals(str) || "\n".equals(str) || "\r".equals(str);
    }
	...
	/**
	 * 生成短信验证码
	 * @param numberFlag 是否生成数字型验证码
	 * @param length 验证码长度
	 * @return
	 */
	public static String getVCode(boolean numberFlag, int length) {
		String checkCode = "";
		...
		do {
			...
			for (int i = 0; i < length; i++) {
				...
				checkCode += strTable.charAt(intR);
			}
			if (count >= 2) {
				bDone = false;
			}
